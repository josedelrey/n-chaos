...
w_max = 0.5
w_values = np.linspace(-w_max, w_max, num_frames)
...
num_samples = int(num_frames / 40)  # Number of sampled points in time for plot limits
...
def find_attractor_4d(search_steps=search_steps, convergence_threshold=convergence_threshold, std_threshold=std_threshold, range_threshold=range_threshold):
    ...
    while not found:
        ...
        for i in range(search_steps):
            ...
            x_prime_new = math.sin(a * y_prime + b * w) - z_prime * math.cos(c * x_prime + d * w)
            y_prime_new = math.sin(e * z_prime + f * w) + x_prime * math.cos(g * y_prime + h * w)
            z_prime_new = math.sin(a * x_prime + b * w) + y_prime * math.cos(c * z_prime + d * w)
            ...
            x_prime, y_prime, z_prime = x_prime_new, y_prime_new, z_prime_new
...
if __name__ == "__main__":
    ...
    # -----------------------
    # Logging Attractor Information to log.csv
    # -----------------------
    log_file = "log.csv"
